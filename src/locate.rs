...
fn find_terminal_for_tty(tty: &str) -> Result<Option<(String, String, Option<u32>)>> {
    ...
    // The pane has its own TTY, but the tmux client is attached to the terminal's TTY.
    // Check if we're in tmux and find the client TTY.
...
fn find_shelldon_tty() -> Option<String> {
    // Walk up to find the shelldon login shell process
    let pid = std::process::id();
    let mut current_pid = pid;
...
fn detect_shelldon() -> Result<Option<Vec<String>>> {
    ...
    // Find the shelldon instance by looking for discovery files matching our parent
    let pid = std::process::id();
    let mut current_pid = pid;

    // Walk up to find a shelldon process
    for _ in 0..20 {
        let output = Command::new("ps")
        ...
        if comm.contains("shelldon") || comm == "-shelldon" {
            // Check discovery files for this PID and its parent (the login shell
            // `-shelldon` is the one that writes discovery files)
            ...
            // No discovery file, just report the PID
